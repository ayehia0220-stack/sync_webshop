...
def _order_phone(sales_order):
	"""
	رقم الزبون اللي نقدر نوصله فعلاً.

	مش أول رقم نلاقيه — أول رقم **صالح**. في طلبات قديمة الرقم الأساسي
	ناقص خانة والرقم التاني سليم؛ الأول كان بيتاخد وترجع الرسالة فاشلة.
	"""
	for field in ("contact_mobile", "custom_customer_phone_number",
	              "contact_phone", "webshop_phone_alt",
	              "custom_second_customer_phone_number"):
		value = (sales_order.get(field) or "").strip()
		if value and normalise_msisdn(value):
			return value
	return None
...
def _handle_evolution(payload, instance=None):
	...
	# رقم حملة التجديد ليه منطقه الخاص — شجرة اختيارات وصور دفع
	if not handled:
		try:
			from sync_webshop.api.renewal_runner import handle_reply
			handled = handle_reply(payload, instance)
		except Exception:
