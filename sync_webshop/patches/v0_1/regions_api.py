# -*- coding: utf-8 -*-
"""Governorate → area lists, read from the ERP's own Territory tree."""
import io
...
def execute():
	...
	# The picker must refresh when the owner edits a zone or the tree.
	h = "/home/frappe/frappe-bench-15/apps/sync_webshop/sync_webshop/hooks.py"
	s = io.open(h, encoding="utf-8").read()
	if "clear_regions_cache" not in s and "doc_events" in s:
		s = s.replace(
			"doc_events = {",
			'doc_events = {\n'
			'\t"Territory": {\n'
		...
		print("hooks: cache invalidation")

	frappe.cache().delete_value("webshop_regions")
	frappe.db.commit()
	frappe.clear_cache()
	print("REGIONS API READY")
